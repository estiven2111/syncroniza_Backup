...
const consultaIndicadoresController_ok2 = async (req, res) => {
  const { docId } = req.query;
  const anio = Number(req.query.anio);
  const mes = Number(req.query.mes);
...
SELECT
    ...
    -- ==========================
    -- ATRASO (🔥 CORREGIDO FINAL)
    -- ==========================
    ROUND(AVG(CASE
        WHEN Año = :anio
         AND mes = :mes
        THEN TRY_CAST(
            REPLACE(REPLACE([% Atraso], ',', ''), '%', '')
            AS DECIMAL(18,6)
        )
        ELSE NULL
    END),1) AS Atraso_Mes,
    ...
    -- ==========================
    -- COMPLETADO (🔥 CORREGIDO FINAL)
    -- ==========================
    ROUND(AVG(CASE
        WHEN Año = :anio
         AND mes = :mes
        THEN TRY_CAST(
            REPLACE(REPLACE([Nivel de Desempeño], ',', ''), '%', '')
            AS DECIMAL(18,6)
        )
        ELSE NULL
    END),1) AS Completado_Mes,
